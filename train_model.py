...
#
# data = torch.load("processed_labeled_dataset.pt")
# labels = sorted({x[2] for x in data})
# label_map = {l: 1 for i, l in enumerate(labels)}
#
# class TrainModel:
#     def __init__(self, train_path="train_dataset.pt",test_path='test_dataset.pt', batch_size=4, epochs=5):
#         self.train_data = torch.load(train_path)
#         self.test_data = torch.load(test_path)
#         self.labels = sorted({x[2] for x in self.train_data})
...
#             base_model,
#             nn.Flatten(),
#             nn.Linear(512, len(self.label_map))
#         ).to(self.device)
#
...
#             _, _, label, frames = data[i]
#             x = torch.tensor(frames, dtype=torch.float32).permute(0,3,1,2)
#             return x, torch.tensor(self.label_map[label], dtype=torch.long)
#
#
#     def train_model(self):
#         print("Training Started")
#         train_loader = DataLoader(self.VideoDS(self.train_data, self.label_map), batch_size=self.batch_size, shuffle=True)
...
#                 B, T, C, H, W = x.shape
#                 feats = self.model[0](x.view(B*T, C, H, W)).view(B, T, -1).mean(1)
#                 out = self.model[2](feats)
...
#         self.model.eval()
#         test_loader = DataLoader(self.VideoDS(self.test_data, self.label_map), batch_size=1, shuffle=True)
#         predictions = []
#         while torch.no_grad():
...
#                 label = self.inv_label_map[pred]
#                 true_label = self.inv_label_map[y.item()]
#                 predictions.append((true_label, label))
#                 print(f"True: {true_label} | Predicted: {label}")
#         return predictions
